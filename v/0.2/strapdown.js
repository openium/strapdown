...
(function () {
    ...
    block._tag = '(?!(?:a|em|strong|small|s|cite|q|dfn|abbr|data|time|code|var|samp|kbd|sub|sup|i|' +
            'b|u|mark|ruby|rt|rp|bdi|bdo|span|br|wbr|ins|del|img)\\b)\\w+(?!:/|@)\\b';
    ...
    Lexer.prototype.token = function (src, top) {
        ...
        while (src) {
            ...
            if (cap = this.rules.fences.exec(src)) {
                ...
                this
                    .tokens
                    .push({type: 'code', lang: cap[2], text: cap[3]});
                continue;
            }
            if (cap = this.rules.heading.exec(src)) {
                ...
                this
                    .tokens
                    .push({type: 'heading', depth: cap[1].length, text: cap[2]});
                continue;
            }
            ...
            if (cap = this.rules.blockquote.exec(src)) {
                ...
                this
                    .tokens
                    .push({type: 'blockquote_start'});
                cap = cap[0].replace(/^ *> ?/gm, '');
                this.token(cap, top);
                this
                    .tokens
                    .push({type: 'blockquote_end'});
                continue;
            }
    ...
    InlineLexer.prototype.outputLink = function (cap, link) {
        if (cap[0][0] !== '!') {
            ...
            return '<img src="' + escape(link.href) + '" alt="' + escape(cap[1]) + '"' + (link.title
                ? ' title="' + escape(link.title) + '"'
                : '') + '>';
        }
    };
    ...
    Parser.prototype.tok = function () {
        switch (this.token.type) {
            ...
            case 'heading':
                {
                    return '<h' + this.token.depth + '>' + this
                        .inline
            ...
            case 'code':
                {
                    if (this.options.highlight) {
                        var code = this
                            .options
                            .highlight(this.token.text, this.token.lang);
                    ...
                    return '<pre><code' + (this.token.lang
    ...
    .call(function () {
        return this || (typeof window !== 'undefined'
            ? window
            : global);
    }());
...
(function () {
    ...
    var C_KEYWORDS = [
        FLOW_CONTROL_KEYWORDS, "auto,case,char,const,default,double,enum,extern,float,goto,int,long,register,sho" +
                "rt,signed,sizeof,static,struct,switch,typedef,union,unsigned,void,volatile"
    ];
    var COMMON_KEYWORDS = [
        C_KEYWORDS, "catch,class,delete,false,import,new,operator,private,protected,public,this,throw" +
                ",true,try,typeof"
    ];
    var CPP_KEYWORDS = [
        COMMON_KEYWORDS, "alignof,align_union,asm,axiom,bool,concept,concept_map,const_cast,constexpr,decl" +
                "type,dynamic_cast,explicit,export,friend,inline,late_check,mutable,namespace,nul" +
                "lptr,reinterpret_cast,static_assert,static_cast,template,typeid,typename,using,v" +
                "irtual,where"
    ];
    var JAVA_KEYWORDS = [
        COMMON_KEYWORDS, "abstract,boolean,byte,extends,final,finally,implements,import,instanceof,null,na" +
                "tive,package,strictfp,super,synchronized,throws,transient"
    ];
    var CSHARP_KEYWORDS = [
        JAVA_KEYWORDS, "as,base,by,checked,decimal,delegate,descending,dynamic,event,fixed,foreach,from," +
                "group,implicit,in,interface,internal,into,is,let,lock,object,out,override,orderb" +
                "y,params,partial,readonly,ref,sbyte,sealed,stackalloc,string,select,uint,ulong,u" +
                "nchecked,unsafe,ushort,var,virtual,where"
    ];
    var COFFEE_KEYWORDS = "all,and,by,catch,class,else,extends,false,finally,for,if,in,is,isnt,loop,new,no," +
            "not,null,of,off,on,or,return,super,then,throw,true,try,unless,until,when,while,y" +
            "es";
    var JSCRIPT_KEYWORDS = [COMMON_KEYWORDS, "debugger,eval,export,function,get,null,set,undefined,var,with,Infinity,NaN"];
    var PERL_KEYWORDS = "caller,delete,die,do,dump,elsif,eval,exit,foreach,for,goto,if,import,last,local," +
            "my,next,no,our,print,package,redo,require,sub,undef,unless,until,use,wantarray,w" +
            "hile,BEGIN,END";
    var PYTHON_KEYWORDS = [
        FLOW_CONTROL_KEYWORDS, "and,as,assert,class,def,del,elif,except,exec,finally,from,global,import,in,is,la" +
                "mbda,nonlocal,not,or,pass,print,raise,try,with,yield,False,True,None"
    ];
    var RUBY_KEYWORDS = [
        FLOW_CONTROL_KEYWORDS, "alias,and,begin,case,class,def,defined,elsif,end,ensure,false,in,module,next,nil" +
                ",not,or,redo,rescue,retry,self,super,then,true,undef,unless,until,when,yield,BEG" +
                "IN,END"
    ];
    var SH_KEYWORDS = [FLOW_CONTROL_KEYWORDS, "case,done,elif,esac,eval,fi,function,in,local,set,then,until"];
    var ALL_KEYWORDS = [
        CPP_KEYWORDS, CSHARP_KEYWORDS, JSCRIPT_KEYWORDS, PERL_KEYWORDS + PYTHON_KEYWORDS,
        RUBY_KEYWORDS,
        SH_KEYWORDS
    ];
    ...
    var REGEXP_PRECEDER_PATTERN = '(?:^^\\.?|[+-]|[!=]=?=?|\\#|%=?|&&?=?|\\(|\\*=?|[+\\-]=|->|\\/=?|::?|<<?=?|>>?>?' +
            '=?|,|;|\\?|@|\\[|~|{|\\^\\^?=?|\\|\\|?=?|break|case|continue|delete|do|else|fina' +
            'lly|instanceof|return|throw|try|typeof)\\s*';
    function combinePrefixPatterns(regexs) {
        ...
        function caseFoldCharset(charSet) {
            var charsetParts = charSet
                ...
                .match(new RegExp('\\\\u[0-9A-Fa-f]{4}|\\\\x[0-9A-Fa-f]{2}|\\\\[0-3][0-7]{0,2}|\\\\[0-7]{1,2}|\\\\[' +
                        '\\s\\S]|-|[^-\\\\]',
                'g'));
            var ranges = [];
        ...
        function allowAnywhereFoldCaseAndRenumberGroups(regex) {
            var parts = regex
                .source
                .match(new RegExp('(?:\\[(?:[^\\x5C\\x5D]|\\\\[\\s\\S])*\\]|\\\\u[A-Fa-f0-9]{4}|\\\\x[A-Fa-f0-9]{2}' +
                        '|\\\\[0-9]+|\\\\[^ux0-9]|\\(\\?[:!=]|[\\(\\)\\^]|[^\\x5B\\x5C\\(\\)\\^]+)',
                'g'));
            var n = parts.length;
            var capturedGroups = [];
        ...
        return new RegExp(rewritten.join('|'), ignoreCase
            ? 'gi'
            : 'g');
    }
    ...
    function sourceDecorator(options) {
        ...
        if (options['regexLiterals']) {
            var REGEX_LITERAL = ('/(?=[^/*])(?:[^/\\x5B\\x5C]|\\x5C[\\s\\S]|\\x5B(?:[^\\x5C\\x5D]|\\x5C[\\s\\S])*(' +
                    '?:\\x5D|$))+/');
        ...
        fallthroughStylePatterns.push([
            ...
            PR_LITERAL,
            new RegExp('^(?:0x[a-f0-9]+|(?:\\d(?:_\\d+)*\\d*(?:\\.\\d*)?|\\.\\d\\+)(?:e[+\\-]?\\d+)?)[a-' +
                    'z]*',
            'i'),
            null,
            '0123456789'
    ...
    registerLangHandler(sourceDecorator({'keywords': CPP_KEYWORDS, 'hashComments': true, 'cStyleComments': true, 'types': C_TYPES}), [
        'c',
        'cc',
        'cpp',
        'cxx',
    ...
    registerLangHandler(sourceDecorator({'keywords': CSHARP_KEYWORDS, 'hashComments': true, 'cStyleComments': true, 'verbatimStrings': true, 'types': C_TYPES}), ['cs']);
                ...
                if (prettyPrintRe.test(className) && !prettyPrintedRe.test(className)) {
                    ...
                    if (!nested) {
                        ...
                        if (preformattedTagNameRe.test(cs.tagName)) {
                            ...
                            var whitespace = (currentStyle
                                ...
                                : (document.defaultView && document.defaultView.getComputedStyle)
                                    ? document.defaultView.getComputedStyle(cs, null).getPropertyValue('white-space')
                                    : 0);
                        ...
                        var lineNums = cs
                            ...
                            .match(/\blinenums\b(?::(\d+))?/);
                        lineNums = lineNums
                            ? lineNums[1] && lineNums[1].length ?+ lineNums[1]
                            : true : false;
                        if (lineNums) {
...
(function (window, document) {
    ...
    container.id = 'markdownContent';
    markdownEl.parentNode
        .replaceChild(container, markdownEl);
...
/*
    ...
    if (!navbarEl && titleEl) {
        navbar.innerHTML = '<div class="navbar-inner"> <div class="container"> <div id="headline" class="bra' +
                'nd"> </div> </div> </div>';
        document
            .body
            .insertBefore(navbar, document.body.firstChild);
...
/*
    ...
    header.className = 'openium-header';
    header.id = 'openium-header';
    container.parentNode.insertBefore(header, container);

    var header1 = document.createElement('div');
    header1.className = 'openium-header-1 openium-header-divided';
    header.appendChild(header1)

    var header2 = document.createElement('div');
    header2.className = 'openium-header-2 openium-header-divided';
    header.appendChild(header2)

    var header3 = document.createElement('div');
    header3.className = 'openium-header-3 openium-header-divided';
    header.appendChild(header3)

    var header4 = document.createElement('div');
    header4.className = 'openium-header-4 openium-header-divided';
    header.appendChild(header4)

    var header5 = document.createElement('div');
    header5.className = 'openium-header-5 openium-header-divided';
    header.appendChild(header5)

    var header6 = document.createElement('div');
    header6.className = 'openium-header-6 openium-header-divided';
...
/*
    ...
    image.className = 'openium-logo';
    image.id = 'openium-logo';
    image.src = originBase + '/images/logo-openium.png';
    image.width = '200';
    ...
    float.className = 'openium-header-right';
    float.id = 'openium-header-right';
    container.insertBefore(float, image.nextSibling)

    // Get client
    var customer = document.createElement('span');
    customer.className = 'openium-customer';
    customer.id = 'openium-customer';
    customer.innerHTML = markdownEl.getAttribute('customer') || '';
    float.appendChild(customer)

    // Specs
    var specs = document.createElement('span');
    specs.className = 'openium-specs';
    specs.id = 'openium-specs';
    specs.innerHTML = '<br />Spécifications fonctionnelles';
    float.appendChild(specs)
*/
    // Prettify
